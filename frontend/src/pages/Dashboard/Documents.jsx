...
function StatCard({ label, value, highlight }) {
  return (
    <div className={`rounded-[20px] border p-5 flex flex-col gap-1 min-w-[120px] ${highlight ? "border-[#ED8D31] bg-[#FFF8F2]" : "border-[#E5E2DC] bg-white"}`}>
      <span className={`text-[28px] font-extrabold leading-none ${highlight ? "text-[#ED8D31]" : "text-[#2F343B]"}`}>{value}</span>
...
function StatusBadge({ status }) {
  const map = {
    missing: { label: "Missing", cls: "bg-[#FEF3F2] text-[#D92D20] border-[#FEE4E2]" },
    uploaded: { label: "Uploaded", cls: "bg-[#F0F9FF] text-[#026AA2] border-[#B9E6FE]" },
    verified: { label: "Verified", cls: "bg-[#E9F7EF] text-[#2F8C57] border-[#A6E0BE]" },
  };
  const { label, cls } = map[status] || map.missing;
  return (
    <span className={`inline-flex px-3 py-1 rounded-full text-xs font-semibold border ${cls}`}>{label}</span>
  );
}
    ...
    <div className="rounded-[20px] border border-[#E5E2DC] bg-white p-5 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
      <div className="flex flex-col gap-1 flex-1">
        <div className="flex items-center gap-2 flex-wrap">
          <span className="text-xs font-medium text-[#7A8088] bg-[#F7F7F5] border border-[#E5E2DC] px-3 py-0.5 rounded-full">{doc.activity}</span>
          <span className="text-xs font-semibold text-[#2F8C57] bg-[#E9F7EF] border border-[#A6E0BE] px-3 py-0.5 rounded-full">{doc.activityTag}</span>
        </div>
        <h4 className="text-[#2F343B] font-bold text-[15px] mt-1">{doc.name}</h4>
        <p className="text-sm text-[#7A8088]">
          {doc.due ? <><span className="text-[#D92D20] font-medium">Due by: {doc.due}</span>{" · "}{doc.note}</> : doc.note}
        </p>
        {doc.fileName && (
      ...
      <div className="flex items-center gap-3 shrink-0">
        ...
        {doc.status === "missing" && (
          <button onClick={() => onUpload(doc.id)} className="flex h-[38px] px-5 items-center rounded-full bg-[#ED8D31] text-white text-sm font-semibold hover:bg-[#d97d22] transition-colors">Upload</button>
        )}
        {doc.status === "uploaded" && (
          <button className="flex h-[38px] px-5 items-center rounded-full border border-[#E5E2DC] bg-white text-[#2F343B] text-sm font-medium hover:bg-gray-50 transition-colors">Replace</button>
        )}
        {doc.status === "verified" && (
          <button className="flex h-[38px] px-5 items-center rounded-full border border-[#E5E2DC] bg-white text-[#7A8088] text-sm font-medium hover:bg-gray-50 transition-colors">View</button>
        )}
      </div>
    </div>
  );
...
function UploadModal({ docName, onClose, onConfirm }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur-[2px] px-4">
      <div className="w-full max-w-[480px] bg-white rounded-[28px] border border-[#E5E2DC] p-8 shadow-xl">
        <h3 className="text-[#2F343B] text-xl font-bold mb-1">Upload Document</h3>
        ...
        <label className="flex flex-col items-center justify-center w-full h-[140px] rounded-[18px] border-2 border-dashed border-[#E5E2DC] bg-[#F7F7F5] cursor-pointer hover:border-[#ED8D31] transition-colors">
          ...
          {file ? (
            <div className="text-center px-4">
              ...
              <p className="text-[#7A8088] text-xs mt-1">{(file.size / 1024).toFixed(1)} KB</p>
            </div>
          ) : (
            <div className="text-center px-4">
              <svg className="mx-auto mb-2" width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="#7A8088" strokeWidth="1.5">
                <path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4" />
                <polyline points="17 8 12 3 7 8" />
                <line x1="12" y1="3" x2="12" y2="15" />
              </svg>
              <p className="text-[#7A8088] text-sm">Click to choose a file or drag it here</p>
              <p className="text-[#7A8088] text-xs mt-1">PDF, JPG, PNG · Max 5 MB</p>
            </div>
          )}
        </label>
        <div className="flex gap-3 mt-6">
          <button onClick={onClose} className="flex-1 py-3 rounded-[14px] border border-[#E5E2DC] bg-white text-[#2F343B] text-sm font-medium hover:bg-gray-50 transition-colors">Cancel</button>
          <button onClick={() => file && onConfirm(file)} disabled={!file} className="flex-1 py-3 rounded-[14px] bg-[#ED8D31] text-white text-sm font-semibold disabled:opacity-40 hover:bg-[#d97d22] transition-colors">Submit</button>
        </div>
      </div>
    </div>
  );
...
export default function Documents() {
  ...
  return (
    <DashboardLayout>
        ...
        <div className="flex gap-1 bg-white border border-[#E5E2DC] rounded-full p-1 w-fit">
          {tabs.map((tab) => (
            <button
              ...
              className={`px-4 py-1.5 rounded-full text-sm font-medium transition-colors ${activeTab === tab.key ? "bg-[#2F343B] text-white" : "text-[#7A8088] hover:text-[#2F343B]"}`}
            >
              {tab.label}
            </button>
          ))}
      ...
      {uploadTarget && (
        <UploadModal
          docName={docs.find((d) => d.id === uploadTarget)?.name}
